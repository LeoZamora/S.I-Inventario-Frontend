...
export default function TipoSolicitudCreate({ open = false, isEdit, onClose, onClick, idTipoSolicitud }: Props) {
    const {
    ...
    const title = isEdit ? 'Editar Tipo Solicitud' : 'Nuevo Tipo Solicitud'
    ...
    async function onSubmit(data: FormProps) {
        try {
            ...
            const result = await requestHttp.postTipoSolicitud(dataForm)
    ...
    useEffect(() => {
        ...
        if (isEdit) {
            getTipoProdById(idTipoSolicitud)
        } else {
            reset({
                nombre: '',
                descripcion: '',
