...
import TipoOrdenCreate from './ordenes_conponents/TipoOrdenCreate.tsx';
...
async function getTipoOrden() {
    try {
        const result = await requestGraph.queryGraph(queries.GET_TIPO_ORDEN_COMBOBOX)
...
export default function TiposOrdenes() {
    const [idOrden, setIdTipoProd] = useState(null)
                ...
                return (
                    <GridActionsCell {...params}>
                        ...
                        <GridActionsCellItem
                            ...
                            showInMenu
                            onClick={() => {
                                setIdTipoProd(params.row.idOrden)
                                setOpenCreate(true)
                                setEdit(true)
                            }}
                        />
    ...
    const load = useCallback(async (alive: boolean) => {
        try {
            setLoading(true);
            const result = await getTipoOrden()
            setLoading(false);
            if (result.findAllTipoOrdenes) {
                const data = result.findAllTipoOrdenes
                if (!alive) return
            ...
            <GridChartsIntegrationContextProvider>
                <Fade in>
                    ...
                    >
                        <DataGridPremium
                            ...
                            slotProps={{
                                ...
                                toolbar: {
                                    ...
                                    addButton: (
                                        <Tooltip title="Nuevo Tipo de Orden">
                        ...
                        <TipoOrdenCreate open={openCreate} onClose={() => {
                            setOpenCreate(false)
                            setEdit(false)
                            load(true)
                        ...
                        isEdit={isEdit}  idTipoOrden={idOrden}/>
                    </Box>
                </Fade>
