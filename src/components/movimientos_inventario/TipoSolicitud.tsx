...
import AlertComp from '../../reusable/AlertComp.tsx';
import TipoSolicitudCreate from './solicitudes_components/TipoSolicitudCreate.tsx';
...
async function getTipoSolicitud() {
    try {
        const result = await requestGraph.queryGraph(queries.GET_TIPO_SOLICITUD_COMBOBOX)
...
export default function TiposSolicitudes() {
    const [idTipoSolicitud, setIdTipoProd] = useState(null)
    ...
    const headersColumns: GridColDef[] = useMemo(() => [{
                ...
                return (
                    <GridActionsCell {...params}>
                        ...
                        <GridActionsCellItem
                            ...
                            showInMenu
                            onClick={() => {
                                setIdTipoProd(params.row.idTipoSolicitud)
                                setOpenCreate(true)
                                setEdit(true)
                            }}
                        />
            ...
            resizable: false,
            flex: 1,
            headerName: 'Nombre Tipo Solicitud',
        }, {
            field: 'descripcion',
            headerAlign: 'center',
            align: 'center',
    ...
    const load = useCallback(async (alive: boolean) => {
        try {
            setLoading(true);
            const result = await getTipoSolicitud()
            setLoading(false);
            if (result.findTipoSolicitud) {
                const data = result.findTipoSolicitud
                if (!alive) return
            ...
            <GridChartsIntegrationContextProvider>
                <Fade in>
                    ...
                    >
                        <DataGridPremium
                            ...
                            slotProps={{
                                ...
                                toolbar: {
                                    ...
                                    addButton: (
                                        <Tooltip title="Nuevo Tipo de Solicitud">
                        ...
                        <TipoSolicitudCreate open={openCreate} onClose={() => {
                            setOpenCreate(false)
                            setEdit(false)
                            load(true)
                        }} onClick={(code: number, msg: string) => {
                            handleCreateItem(code, msg)
                        }}
                        isEdit={isEdit}  idTipoSolicitud={idTipoSolicitud}/>
                    </Box>
                </Fade>
